...
Public Class GalaxyGates
    ...
    Public Shared Function View(ByRef GalaxyGates_id, ByRef GalaxyGates_Name)
    ...
    Public Shared Function Spin(ByRef GalaxyGates_id, ByRef GalaxyGates_Name)
        ...
        Try
            ...
            WebClient_POST.Headers.Add(HttpRequestHeader.Cookie, $"dosid={Utils.dosid};")
            Dim WebClient_Data = WebClient_POST.DownloadString("https://" + Utils.server + ".darkorbit.com/flashinput/galaxyGates.php?userID=" + Utils.userid + "&action=multiEnergy&sid=" + Utils.dosid + "&gateID=" + GalaxyGates_id + "&" + GalaxyGates_Name + "=1&sample=1&multiplier=1")
            ...
            Dim Webclient_GET_Items_type = Regex.Match(WebClient_Data, "type=""(.*)"" item_id").Groups.Item(1).ToString ' type de recompense
            Dim Webclient_GET_Items_item_id = Regex.Match(WebClient_Data, "item_id=""(.*)"" spins").Groups.Item(1).ToString ' id de recompense
            Dim Webclient_GET_Items_spins = Regex.Match(WebClient_Data, "spins=""(.*)"" amount").Groups.Item(1).ToString ' nombre de spins effectuer pour ce spin
            Dim Webclient_GET_Items_amount = Regex.Match(WebClient_Data, "amount=""(.*)"" date").Groups.Item(1).ToString ' quantité de recompense
            ...
            ElseIf Webclient_GET_Items_type = "part" Then
                Form_Tools.Label_Part_Earned.Text = Val(Form_Tools.Label_Part_Earned.Text) + Webclient_GET_Items_amount
                Form_Tools.TextBox_WinGGS.Text = vbNewLine + "( " + Webclient_GET_Items_spins + " ) " + Webclient_GET_Gates_name + " :   Part(s) Added" + Form_Tools.TextBox_WinGGS.Text
                Exit Function

            End If

            Form_Tools.TextBox_WinGGS.Text = vbNewLine + "( " + Webclient_GET_Items_spins + " ) " + Webclient_GET_Gates_name + " :   " + Webclient_GET_Items_amount + " " + Spins_reward + Form_Tools.TextBox_WinGGS.Text
            Load()
